...
**Finding 3 — `bhound`'s `adminCount=1` matches its only current group after all.** `bhound` — created 2026-06-17T07:03:56Z, matching the BloodHound write-up's stated capstone session exactly — carries `adminCount: 1`, and its only listed `MemberOf` is `Key Admins`. At the time this was written, Key Admins looked like it wasn't on the classic AdminSDHolder-protected-groups list, leaving two live, unconfirmed explanations. **Resolved 2026-09-01: it is protected** — Microsoft Learn's Windows Server security best practices (Appendix C, Protected Accounts and Groups) lists both Key Admins and Enterprise Key Admins alongside Domain Admins and the rest. The first explanation was right; there was no mystery membership to find. **Captured** as a raw fact (the mismatch, now explained); see `verified-claims.md` for the on-the-record correction.
...
- ~~**`bhound`'s `adminCount=1` origin is unresolved.**~~ **RESOLVED 2026-09-01** — see the corrected Finding 3 above. Key Admins is AdminSDHolder-protected; that alone explains it.
- ~~**Should `bhound` be disabled or removed?**~~ **RESOLVED 2026-09-01.** A follow-up capture confirmed Key Admins actually holds domain-wide write rights over `msDS-KeyCredentialLink` in this domain (not just a documented theoretical risk), making `bhound` a live shadow-credentials path. Raymond's call was both: removed from Key Admins and disabled. See `exercises/2026-09-01-entra-connect-connector-account/report.md`.
