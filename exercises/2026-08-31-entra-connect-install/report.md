...
- **Entra Connect is not installed.** Components are on VM 102; no sync is configured, no directory connected, no object has ever synced. The partial install is itself a state worth knowing about — a future attempt starts from a half-configured machine, not a clean one.
- **The one untested hypothesis: stale wizard state.** A full application exit and relaunch was recommended and never tried. Everything the wizard needs is now demonstrably working — DNS, SRV/locator, ports 389/3268/443, valid enabled credentials, signing lowered — which makes "the wizard cached an early failure" the most plausible remaining explanation, and the cheapest thing to test first next time.
...
- **Residual from the disclosure: the Administrator password is still the value that entered this conversation.** It is *neutralized* — a disabled account can't authenticate — but it is not *rotated*. If that account is ever re-enabled for any reason, rotate the password first, using an interactive `Set-ADAccountPassword -Identity Administrator -Reset` (no `-NewPassword` argument) from DC01's console so the secret never lands on a command line or in shell history.
...
- **`bhound` still has no UPN and would fail sync.** Carried forward unresolved from the hybrid-identity exercise; it needs a UPN, exclusion from sync scope, or deletion.
- **`districtsafetyphoto.com` verification never happened**, and the registration window is roughly a month.
- **The AD DS connector account was never created**, so the more least-privilege-correct path — pre-provisioning a minimally-scoped sync account by hand instead of handing the wizard Enterprise Admin credentials — remains an untried and genuinely on-thesis follow-up exercise.
